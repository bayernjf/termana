...
fn resolve_and_launch(cfg: &config::Config, project_id: &str) -> Result<(), String> {
    ...
    if project.context.as_ref().map_or(false, |c| !c.trim().is_empty()) {
        let _ = write_context_file(project);
    }
...
fn context_file_for(agent_id: &str) -> &str {
    match agent_id {
        "claude" => "CLAUDE.md",
...
/// Write a project's context to its bound agent's context file in the project
/// directory. Shared by sync_context (manual) and resolve_and_launch
/// (auto-sync on launch). Returns the written path.
fn write_context_file(project: &config::Project) -> Result<std::path::PathBuf, String> {
    let context = project
        .context
        .as_ref()
        .filter(|c| !c.trim().is_empty())
        .ok_or_else(|| "context is empty; write a context first".to_string())?;
    let file = context_file_for(&project.agent);
...
/// Write the project's context to the bound agent's context file (manual sync).
/// Returns the written path.
#[tauri::command]
pub fn sync_context(project_id: String) -> Result<String, String> {
    let cfg = config::load();
    let project = cfg
        .projects
        .iter()
        .find(|p| p.id == project_id)
        .ok_or_else(|| format!("project not found: {}", project_id))?;
    let path = write_context_file(project)?;
